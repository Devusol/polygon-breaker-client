...
import { updateSelectedCardIndexFlip, updateFlipCards, updateActiveCardId, fetchFCActions } from '../../redux'

const FlipCardsScreen = props => {
    ...
    const getFCData = () => {
        ...
        setLoading(true);
        get("flipcards/bycategory/" + categoryId + "/" + qty + "/" + type)
            .then((result) => {
                // console.log(result.data);
                if (result.data.length > 0) {
                    dispatch(updateSelectedCardIndexFlip(selectedIndex))
                    dispatch(updateFlipCards(result.data))
                    setTimeout(() => {
                        setLoading(false);
                        props.navigation.navigate("FlipCardAction")
                    }, 1500)
